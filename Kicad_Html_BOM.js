    ...
    for ( var Group in groupedList )
    {
        ...
        for ( var Item in partGroupedList[GroupdName] )
        {
            ...
            for ( var FieldIndex = 0; FieldIndex <  fieldsList.length; FieldIndex++ )
            {
                ...
                SingleFieldTemp = SingleFieldTemp.replace(/<!--TAG_BOM_TABLE_FIELD_CLASS_TAG-->/g, "Field_" + fieldsList[ FieldIndex ].replace(/\s+/g, '_').replace(/\./g, '_').replace(/_$/, '') );

                if( partGroupedList[ GroupdName ][ Item ].Fields[ fieldsList[ FieldIndex ] ] )
                {
                    if (fieldsList[ FieldIndex ] == "Supplier No.")
                    {
                        var fieldValue = partGroupedList[ GroupdName ][ Item ].Fields[ fieldsList[ FieldIndex ] ].replace(/,/g, " ");

                        SingleFieldTemp = SingleFieldTemp.replace(/<!--TAG_BOM_TABLE_FIELD-->/g, '<a href="http://www.findchips.com/search/' + fieldValue + '" target="_blank">' + fieldValue + '</a>');
                    }
                    else
                    {
